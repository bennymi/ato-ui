<script lang="ts">
	...
	// : updated_descriptions.filter((v) => filters.some((f) => (<string[]>v.keywords).includes(f)));
...
<div class="grid w-full flex flex-wrap gap-2 mt-4">
	{#each filter_values as key, i}
		<div class="min-w-40 flex-1">
			...
			>
				<div class="w-full text-center text-lg font-semibold">{key}</div>
			</label>
		</div>
	{/each}
</div>
...
<div class="text-surface-900-50 flex flex-col gap-4">
	{#each searched_list as { rule, description, keywords, variablesUsed, examples }}
		<div
			class="rounded-container p-4 border-2 bg-surface-50-600 backdrop-blur-sm transition-all duration-200 border-surface-100-900 hover:(border-primary-500 bg-mesh-primary-x50-y105-primary-x115-y105-primary-x-15-y105) cursor-pointer"
