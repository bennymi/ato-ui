...
export const entries = (() => {
	const slugs = getGuidesFolderFiles('get-started').map((file) => ({ slug: file }));
	return slugs;
...
export const load = (async ({ params,  fetch }) => {
    ...
    if (!getGuidesFolderFiles('get-started').includes(slug)) {
        throw error(404);
    }
    ...
    const previewSnippets = await getAllPreviewSnippets({ slug, theme: theme ?? 'github-dark', required: false });
