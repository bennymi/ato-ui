...
export const textSCs: Shortcut[] = [
	...
	[
		...
		{
			autocomplete: [`text-${reg_c}-${reg_s}-${reg_s}`]
		}
	],
	...
	[
		new RegExp(
			`^text-${reg_c}-${reg_s}${reg_oO}-${reg_s}${reg_oO}-${reg_s}${reg_oO}-${reg_s}${reg_oO}$`
		),
		([, c, s1, o1, s2, o2, s3, o3, s4, o4]: string[]) =>
			`text-${cso(c, s1, o1)} hover:text-${cso(c, s2, o2)} dark:(text-${cso(
				c,
				s3,
				o3
			)} hover:text-${cso(c, s4, o4)})`,
		{
	...
	[
		...
		([, c1, s1, o1, c2, s2, o2]: string[]) =>
			`text-${cso(c1, s1, o1)} dark:text-${cso(c2, s2, o2)}`,
		{
			autocomplete: [
				'text-inverse',
				`text-inverse-${reg_c}`,
	...
	[
		new RegExp(
			`^text-inverse-${reg_c_s}${reg_oO}-${reg_s}${reg_oO}-${reg_c_s}${reg_oO}-${reg_s}${reg_oO}$`
		),
		([, c1, s11, o11, s12, o12, c2, s21, o21, s22, o22]: string[]) =>
			`text-${cso(c1, s11, o11)} hover:text-${cso(c1, s12, o12)} dark:(text-${cso(
				c2,
				s21,
				o21
			)} hover:text-${cso(c2, s22, o22)})`
	],

	// Text gradients
	[
		new RegExp(`^text-gradient${reg_dO}-${reg_c_sO}-${reg_c_sO}$`),
		([, d, c1, s1, c2, s2]: string[]) =>
			`${gradientClasses} bg-gradient-to-${d ?? default_dir} from-${cs(c1, s1)} to-${cs(c2, s2)}`
	],
	[
		new RegExp(`^text-gradient${reg_dO}-${reg_c_sO}-${reg_c_sO}-${reg_c_sO}$`),
		([, d, c1, s1, c2, s2, c3, s3]: string[]) =>
			`${gradientClasses} bg-gradient-to-${d ?? default_dir} from-${cs(c1, s1)} via-${cs(
				c2,
				s2
			)} to-${cs(c3, s3)}`,
		{
